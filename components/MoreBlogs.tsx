...
const MoreBlogs = ({ post }: { post: any }) => {
  ...
  <div className="relative">
    ...
    <div className="absolute top-3 left-3 flex space-x-2">
      ...
      <span className="bg-purple-500 text-white text-xs font-medium px-2 py-1 rounded-md shadow-md">
        {views} views
      </span>
    </div>
  </div>
  ...
  <div className="px-6 pb-4 flex items-center justify-between">
    <div className="flex items-center">
      ...
      <div className="ml-3">
        ...
        <span className="block text-xs ">{new Date(_createdAt).toLocaleDateString()}</span>
      </div>
    </div>
    <Link href={`/blogs/${_id}`}>
